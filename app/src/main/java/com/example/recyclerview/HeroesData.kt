...
object HeroesData {
    ...
    private val heroDetails = arrayOf(
        ...
        "Jenderal Besar Raden Soedirman (EYD: Sudirman; lahir 24 Januari 1916 – meninggal 29 Januari 1950 pada umur 34 tahun) adalah seorang perwira tinggi Indonesia pada masa Revolusi Nasional Indonesia. Menjadi panglima besar Tentara Nasional Indonesia pertama, ia secara luas terus dihormati di Indonesia.",
    ...
    val listData: ArrayList<Hero>
        get() {
            ...
            for (position in heroNames.indices) {
                val hero = Hero(
                    photo = heroesImages[position],
                    detail = heroDetails[position],
                    name = heroNames[position]
                )
                list.add(hero)
            }
            return list
